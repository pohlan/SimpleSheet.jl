using Printf, LinearAlgebra, Statistics, Suppressor
...
@views function simple_sheet(;  nx, ny,                 # grid size
                                ...
                                dτ_h=1.6e-5,             # pseudo-time step for h
    ...
    while iter<itMax && max(err_ϕ, err_h) > tol && !any(isnan.([err_ϕ, err_h]))
        ...
        dϕdτ  .= - Ψ * (inn(ϕ) .- inn(ϕ0)) ./ dt  .+ dϕdt_ev
        dhdτ  .= - (inn(h) .- inn(h0)) ./ dt .+ dhdt

        dϕdτ[1, :] .= 0.      # Dirichlet B.C. points, no update
        ...
        inn(h) .= (inn(h0) ./ dt .+ Σ * (inn(h).<1.0))./(1.0./dt .+ Γ * (inn(H) .- inn(ϕ)).^3 .+ Σ * (inn(h) .< 1.0))
        ...
        if iter % nout == 0
            ...
            err_h = maximum(dhdτ) / sqrt(length(dhdτ))
            err_ϕ = maximum(dϕdτ) / sqrt(length(dϕdτ))
            push!(iters, iter)
            push!(errs_h, err_h)
            push!(errs_ϕ, err_ϕ)
            ...
            if do_monit
                ...
                @suppress begin
                    ...
                    # else
                    Plt.display(Plt.plot(p1, p2, p5, p6))
                    # end
                end
            end
        end
 ...
 ϕ, h, t_sol = simple_sheet(; nx=64, ny=32, e_v_num=0., γ=0.8, dτ_h =1e-3, do_monit=true, itMax=10^5, tol=1e-8)
