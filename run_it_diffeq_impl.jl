...
# # fails:
# using Sundials
# ## @time sol = solve(prob, CVODE_BDF(linear_solver=:GMRES)) #solver, isoutofdomain=(u,p,t) -> any(u.x[1].<0))
# @time sol = solve(prob, ROCK4()) #solver, isoutofdomain=(u,p,t) -> any(u.x[1].<0))
...
tspan = (0, 9.2*day / scales.t_)
u0 = ArrayPartition(copy(h0), copy(ϕ0))
du0 = ArrayPartition(copy(h0), copy(ϕ0))
prob = ODEProblem(f2, u0, tspan, (1,));
println("Running implicit solver for tspan=$tspan")
@time sol = solve(prob, TRBDF2(autodiff=true, chunk_size=chunksize, linsolve=LinSolveGMRES())) # 8.352 s (1875298 allocations: 78.86 MiB)
